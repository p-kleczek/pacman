...
public class Level {
		...
		for (int i = 0; i < height; ++i) {
			for (int j = 0; j < width; ++j) {
				...
				case PLAYER_SPAWN:
					...
					break;
				case POWER_SPAWN:
					powerSpawns.add(new Rect(r.left, r.top, r.left + powerW,
							r.top + powerH));
					break;
				case GOLD_SPAWN:
					goldSpawns.add(new Rect(r.left, r.top, r.left + goldH,
							r.top + goldH));
					break;
	...
	public MathVector randomPlayerSpawn() {
		...
		if (s != 0)
			r = playerSpawns.get(random.nextInt(s));
		else
	...
	public MathVector randomEnemySpawn() {
		...
		if (s != 0)
			r = enemySpawns.get(random.nextInt(s));
		else
			return new MathVector(0, 0);
